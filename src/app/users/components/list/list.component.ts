...
import { INewUser, IUser, IUserQueryParams, IUserRole } from '../../models/user';
...
export class UsersListComponent implements OnInit{
  ...
  public deleteUser(user: IUser) {
    this.dialog.open<ConfirmDialogComponent, IConfirmDialogData>(ConfirmDialogComponent, {
      width: '580px',
      closeOnNavigation: true,
      panelClass: 'confirm-dialog-component',
      data: {
        header: 'Удалеоение пользователя',
        text: `Вы точно хотите удалить пользователя ${user.firstName} ${user.lastName}?`,
      }
    })
    .afterClosed()
    .flatMap(result => {
